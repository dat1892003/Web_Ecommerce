...
namespace SV21T1020123.BusinessLayers
{
    public static class OrderDataService
    {
        ...
        public static List<Order> CustomerListOrders(out int rowCount, int page = 1, int pageSize = 0,
        int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "", int customerId = 0)
        {
            rowCount = orderDB.CustomerOrderCount(status, fromTime, toTime, searchValue, customerId);
            return orderDB.CustomerOrderList(page, pageSize, status, fromTime, toTime, searchValue, customerId).ToList();
        }
        /// <summary>
        ...
        /// <summary>
        /// Khởi tạo 1 đơn hàng từ phía khách hàng
        /// </summary>
        public static int CustomerInitOrder(int customerID, string deliveryProvince, string deliveryAddress, IEnumerable<OrderDetail> details)
        {
            if (details.Count() == 0)
                return 0;
            Order data = new Order()
        ...
        public static bool CancelOrder(int orderID, int employeeID)
        {
            ...
            if (string.IsNullOrEmpty(data.EmployeeID.ToString()))
            {
